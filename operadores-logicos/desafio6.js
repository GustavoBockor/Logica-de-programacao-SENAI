...
let pedido = {
    ...
    regiao: ler.question("Regiao: "),
    valorProdutos: ler.questionFloat("Valor dos produtos: "),
    embalagemPresente: ler.keyInYN("Deseja embalagem presente? ")
};

pedido.freteBase =
    pedido.regiao === "sul" || pedido.regiao === "sudeste"
    ? 15
    : pedido.regiao === "nordeste" || pedido.regiao === "centro"
    ? 25
    : 40;

pedido.freteGratis =
    (pedido.valorProdutos > 150 &&
    (pedido.regiao === "sul" || pedido.regiao === "sudeste"))
    || pedido.valorProdutos > 200;

pedido.desconto50 =
    pedido.valorProdutos >= 80 &&
    pedido.valorProdutos <= 150 &&
    !pedido.freteGratis;
...
pedido.freteReal =
    ...
    ? 0
    : pedido.desconto50
    ? pedido.freteBase * 0.5
    : pedido.freteBase;

pedido.taxaEmbalagem =
    pedido.embalagemPresente ? 12 : 0;
...
console.log(
    `Ola ${pedido.nome}! Total do pedido: R$ ${pedido.totalFinal.toFixed(2)} (Frete: R$ ${pedido.freteReal.toFixed(2)} | Embalagem: R$ ${pedido.taxaEmbalagem.toFixed(2)})`
);
...
let vendedor = {
    nome: ler.question("Nome: "),
    salarioFixo: ler.questionFloat("Salario fixo: "),
    metaMensal: ler.questionFloat("Meta mensal: "),
    totalVendido: ler.questionFloat("Total vendido: "),
    totalClientes: ler.questionInt("Total de clientes: "),
    anosNaEmpresa: ler.questionInt("Anos na empresa: ")
};

vendedor.bateuMeta =
    vendedor.totalVendido >= vendedor.metaMensal;

vendedor.percentualMeta =
    (vendedor.totalVendido / vendedor.metaMensal) * 100;

vendedor.percentualComis =
    vendedor.bateuMeta && vendedor.totalClientes >= 10
    ? 8
    : vendedor.bateuMeta && vendedor.totalClientes < 10
    ? 5
...
console.log(
    `${vendedor.nome} | Bruto: R$ ${vendedor.bruto.toFixed(2)} | INSS: R$ ${vendedor.inss.toFixed(2)} | Liquido: R$ ${vendedor.liquido.toFixed(2)}`
);
...
let investidor = {
    ...
    rendaMensal: ler.questionFloat("Renda mensal: "),
    patrimonio: ler.questionFloat("Patrimonio: "),
    possuiDividas: ler.keyInYN("Possui dividas? "),
    temExperiencia: ler.keyInYN("Tem experiencia em investimentos? ")
};

investidor.ptIdade =
    investidor.idade < 30
    ? 20
    : investidor.idade >= 30 && investidor.idade <= 50
    ? 10
    : 0;

investidor.ptRenda =
    investidor.rendaMensal > 10000
    ? 30
    : investidor.rendaMensal >= 3000 &&
      investidor.rendaMensal <= 10000
    ? 15
    : 0;

investidor.ptPatrimonio =
    investidor.patrimonio > 100000
    ? 25
    : 0;

investidor.ptDividas =
    investidor.possuiDividas ? -30 : 0;

investidor.ptExperiencia =
    investidor.temExperiencia ? 20 : 0;
...
investidor.produto =
    investidor.perfil === "Arrojado"
    ? "Acoes e Fundos de Alto Risco"
    : investidor.perfil === "Moderado"
    ? "Tesouro IPCA+ e Fundos Mistos"
    : "Poupanca e Tesouro Selic";

console.table([investidor]);

console.log(
    `Perfil de ${investidor.nome}: ${investidor.perfil} (Score: ${investidor.score})`
);
...
let segurado = {
    nome: ler.question("Nome: "),
    sexo: ler.question("Sexo (M/F): "),
    idade: ler.questionInt("Idade: "),
    anosContribuidos: ler.questionInt("Anos contribuidos: "),
    mediaSalarial: ler.questionFloat("Media salarial: ")
};

segurado.idadeMinima =
    segurado.sexo === "M" ? 65 : 62;

segurado.contribuicaoMinIdade =
    segurado.sexo === "M" ? 20 : 15;

segurado.contribuicaoPlena =
    segurado.sexo === "M" ? 35 : 30;

segurado.aposentaPorTempo =
    segurado.anosContribuidos >=
    segurado.contribuicaoPlena;

segurado.aposentaPorIdade =
    segurado.idade >= segurado.idadeMinima &&
    segurado.anosContribuidos >=
    segurado.contribuicaoMinIdade;

segurado.podeAposentar =
    segurado.aposentaPorTempo ||
    segurado.aposentaPorIdade;
...
segurado.percentual =
    segurado.aposentaPorTempo
    ? 100
    : 70 + (segurado.excedente * 2);

segurado.beneficioBruto =
    segurado.mediaSalarial *
    (segurado.percentual / 100);

segurado.beneficioFinal =
    segurado.beneficioBruto < 1412
    ? 1412
    : segurado.beneficioBruto > 7786.02
    ? 7786.02
    : segurado.beneficioBruto;
...
console.log(
    segurado.podeAposentar
    ? `Parabens, ${segurado.nome}! Beneficio: R$ ${segurado.beneficioFinal.toFixed(2)} (${segurado.percentual}%)`
    : "Ainda nao elegivel. Faltam anos ou idade minima."
);
...
let fatura = {
    nome: ler.question("Nome: "),
    consumoKwh: ler.questionFloat("Consumo kWh: "),
    rendaPerCapita: ler.questionFloat("Renda per capita: ")
};

fatura.faixa1 =
    (fatura.consumoKwh <= 100
    ? fatura.consumoKwh
    : 100) * 0.40;

fatura.faixa2 =
    (fatura.consumoKwh > 100
    ? (fatura.consumoKwh <= 200
        ? fatura.consumoKwh - 100
        : 100)
    : 0) * 0.55;
...
fatura.iluminacao =
    fatura.subTotal * 0.07;

fatura.icms =
    (fatura.subTotal + fatura.iluminacao) * 0.25;

fatura.ehBaixaRenda =
    fatura.rendaPerCapita <= 218 &&
    fatura.consumoKwh <= 220;

fatura.totalSemDesconto =
    fatura.subTotal +
    fatura.iluminacao +
    fatura.icms +
    4.50;
...
console.log(
    `Fatura de ${fatura.nome} | Consumo: ${fatura.consumoKwh} kWh | Total: R$ ${fatura.totalFinal.toFixed(2)} ${fatura.ehBaixaRenda ? "- Desconto baixa renda aplicado!" : ""}`
);