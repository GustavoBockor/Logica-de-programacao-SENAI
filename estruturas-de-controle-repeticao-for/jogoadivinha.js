...
const modo = ler.questionInt("\nEscolha o modo: ");

console.log("_______________________________");

switch (modo) {
  case 1:
    let secreto = parseInt(Math.random() * 100) + 1;
    let venceu = false;
    let tentativasUsadas = 0;

    console.log("Tente adivinhar o número entre 1 e 100!");
    console.log(`Você possui ${MAX_TENTATIVAS} tentativas.`);
  ...
  case 2:
    ...
    console.log("Não digite o número, apenas responda às perguntas.");

    for (let i = 1; i <= MAX_TENTATIVAS; i++) {
      let palpite = parseInt((min + max) / 2);

      console.log(`\nTentativa ${i}: Meu palpite é ${palpite}!`);

      let resposta = ler.questionInt(
        "1 – Acertei | 2 – Muito alto | 3 – Muito baixo: "
      );

      if (resposta === 1) {
        console.log(`Oba! Acertei em ${i} tentativa(s)!`);
        acertou = true;
        break;
      } else if (resposta === 2) {
        max = palpite - 1;
    ...
    if (!acertou) {
      console.log("Não consegui adivinhar o número.");
    }
