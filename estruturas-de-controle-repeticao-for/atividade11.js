...
console.log("Total de pares:", contador);
...
let N = ler.questionInt("Digite um número (1 a 10): ");
let fatorial = 1;
...
for (let i = 0; i < 3; i++) {
    ...
    let nota = ler.questionFloat("Nota: ");

    turma.push({ nome, nota });
    soma += nota;
}
...
let qtd = ler.questionInt("Quantos produtos? ");
let estoque = [];

for (let i = 0; i < qtd; i++) {
    let nome = ler.question("Nome: ");
    let preco = ler.questionFloat("Preço: ");
